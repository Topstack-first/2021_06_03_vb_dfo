...
Public Class appcode
    ...
    Public Shared ConnectionString As String = "Data Source=localhost;Initial Catalog=SQL2012_972956_desertfleet;Persist Security Info=True;Integrated Security=SSPI;"
    ...
    Public Shared Function ConvertGuestCart(ByVal ssID As String, ByVal vendorID As Integer, ByVal companyID As Integer, ByVal userID As Integer) As Integer
        ...
        ConvertGuestCart = comm.ExecuteNonQuery()
        conn.Close()
    End Function
    ...
    Public Shared Function GetOnOrderAmount(ByVal manufacturer As String, ByVal partnumber As String) As Double
        ...
        commandString = "select (a.sumQty - b.sumShipQty) as onorder, a.sumQty, b.sumShipQty, a.manufacturer, a.partnumber, a.complete from (select sum(quantity) as sumQty, manufacturer, partnumber, complete from [v_quantity_open_orders] group by manufacturer, partnumber, complete) a inner join (select sum(ship_qty) as sumShipQty, manufacturer, partnumber, complete from v_shipment_lines group by manufacturer, partnumber, complete) b on a.manufacturer=b.manufacturer and a.partnumber=b.partnumber and a.complete=b.complete where a.manufacturer=@manufacturer and a.partnumber=@partnumber and a.complete=@complete"
        Dim comm As New SqlCommand(commandString, conn)
        comm.Parameters.AddWithValue("@manufacturer", manufacturer)
        comm.Parameters.AddWithValue("@partnumber", partnumber)
        comm.Parameters.AddWithValue("@complete", False)
    ...
    Public Shared Function UpdateList(ByVal email As String, ByVal name As String, ByVal company As String, ByVal listitemID As Integer) As Integer
        ...
        comm.Parameters.AddWithValue("@company", company)
        comm.Parameters.AddWithValue("@listitemID", listitemID)
        UpdateList = comm.ExecuteNonQuery()
        conn.Close()
    End Function
